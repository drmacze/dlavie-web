...
modern_marker = '// I18N v2 — shared runtime in i18n.js'
end_marker = '// ═══════════════════════════════════════════════════════════════\n// ROUTER'
bridge = '''// ═══════════════════════════════════════════════════════════════
...
if "sw.dataset.mode='register'" not in text:
    text, count = re.subn(
    ...
    if count != 1:
        raise SystemExit('Could not migrate auth switch')

if 'data-i18n="register_google"' not in text:
    text = text.replace(
...
profile_sync = 'window.currentUser=currentUser; window.DLavieI18n?.syncFromProfile(currentUser);'
if profile_sync not in text:
    text = text.replace('window.currentUser=currentUser;', profile_sync)
